# Cover images of one work, paged — the block a store card or a shelf needs on its own · 目录数据 API（只读）
...
Cover images of one work, paged — the block a store card or a shelf needs on its own
...
```bash
curl "https://api.nextmoe.dev/v1/catalog/works/1/covers" \
  -H "Authorization: Bearer nmk_live_<YOUR_KEY>"
```

---
本页来源 · NextMoe 开发者平台 · https://developer.nextmoe.dev/docs/catalog/getCatalogWorkCoversPublic
