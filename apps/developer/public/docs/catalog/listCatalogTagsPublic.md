# Keyset canonical-tag browse lane (id ASC); filter by tier / kind, each row carries an nsfw-aware work_count · 目录数据 API（只读）
...
Keyset canonical-tag browse lane (id ASC); filter by tier / kind, each row carries an nsfw-aware work_count

The cross-source canonical tag vocabulary, id ascending. work_count is the number of works THIS caller would page through via works?tag_id=<id> (counted over the source-tag map, so a work carrying two mapped source tags counts once).
...
---
本页来源 · NextMoe 开发者平台 · https://developer.nextmoe.dev/docs/catalog/listCatalogTagsPublic
