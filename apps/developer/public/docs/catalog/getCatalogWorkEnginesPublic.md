# Engines one work is built with, paged, each with an nsfw-aware work_count · 目录数据 API（只读）
...
Engines one work is built with, paged, each with an nsfw-aware work_count
...
```bash
curl "https://api.nextmoe.dev/v1/catalog/works/1/engines" \
  -H "Authorization: Bearer nm_live_<YOUR_KEY>"
```

---
本页来源 · NextMoe 开发者平台 · https://developer.nextmoe.dev/docs/catalog/getCatalogWorkEnginesPublic
