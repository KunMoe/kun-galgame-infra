# Source tags of one work, paged, with the canonical mapping and the safety axis · 目录数据 API（只读）
...
Source tags of one work, paged, with the canonical mapping and the safety axis
...
| `nsfw` | query | 否 | boolean | true/1 = serve this sub-resource for an r18 work (default false = 404). It also decides work_count: an sfw caller's count excludes the r18 works it can never fetch |
| `spoilers` | query | 否 | integer (int32) | Max tag spoiler level 0-2 (default 0 = safe) — the works/{id} parameter verbatim. Rows above the ceiling are omitted entirely and consume no page slot, so raising the ceiling can change what a given offset points at |

```bash
curl "https://api.nextmoe.dev/v1/catalog/works/1/tags" \
  -H "Authorization: Bearer nm_live_<YOUR_KEY>"
```

---
本页来源 · NextMoe 开发者平台 · https://developer.nextmoe.dev/docs/catalog/getCatalogWorkTagsPublic
