# Series record: identity + source anchor + intros; include=works attaches its member works in reading order · 目录数据 API（只读）
...
Series record: identity + source anchor + intros; include=works attaches its member works in reading order
...
| 参数 | 位置 | 必填 | 类型 | 说明 |
| --- | --- | --- | --- | --- |
| `id` | path | 是 | integer (int64) | Catalog series id |
| `include` | query | 否 | string | works = attach the series' member works |
| `nsfw` | query | 否 | boolean | true/1 = include r18 works among the members (default false = dropped) |
...
---
本页来源 · NextMoe 开发者平台 · https://developer.nextmoe.dev/docs/catalog/getCatalogSeriesPublic
