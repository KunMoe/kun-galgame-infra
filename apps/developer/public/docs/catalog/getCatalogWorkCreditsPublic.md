# Staff credits of one work grouped by role, paged over credit rows · 目录数据 API（只读）
...
Staff credits of one work grouped by role, paged over credit rows
...
```bash
curl "https://api.nextmoe.dev/v1/catalog/works/1/credits" \
  -H "Authorization: Bearer nm_live_<YOUR_KEY>"
```

---
本页来源 · NextMoe 开发者平台 · https://developer.nextmoe.dev/docs/catalog/getCatalogWorkCreditsPublic
