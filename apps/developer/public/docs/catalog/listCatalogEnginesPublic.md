# Keyset engine browse lane (id ASC); each row carries an nsfw-aware work_count · 目录数据 API（只读）
...
Keyset engine browse lane (id ASC); each row carries an nsfw-aware work_count
...
| `nsfw` | query | 否 | boolean | true/1 = count r18 works in work_count (default false = excluded, matching what an sfw works?engine_id= call returns) |
...
---
本页来源 · NextMoe 开发者平台 · https://developer.nextmoe.dev/docs/catalog/listCatalogEnginesPublic
