# Synopses of one work, one per language, paged · 目录数据 API（只读）
...
Synopses of one work, one per language, paged
...
```bash
curl "https://api.nextmoe.dev/v1/catalog/works/1/intros" \
  -H "Authorization: Bearer nmk_live_<YOUR_KEY>"
```

---
本页来源 · NextMoe 开发者平台 · https://developer.nextmoe.dev/docs/catalog/getCatalogWorkIntrosPublic
