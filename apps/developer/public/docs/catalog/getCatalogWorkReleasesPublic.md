# Release rows of one work, paged, each with its exact anchors and its own companies · 目录数据 API（只读）
...
Release rows of one work, paged, each with its exact anchors and its own companies
...
---
本页来源 · NextMoe 开发者平台 · https://developer.nextmoe.dev/docs/catalog/getCatalogWorkReleasesPublic
