...
- **审计**:key 创建/轮换/吊销、tier 变更、异常 4xx/5xx 速率,写审计日志。**2026-08-29 起平台有了两个会真删行的动作**(密钥行、`oauth_clients` 行),两者都被守卫钉在「删掉的东西没有历史可留」这一种情形上——用过的钥匙只能吊销不能删,还有引用的应用只能归档不能删,**曾经能签用户进来的 client 无论如何都只能归档**——过期 session 每小时被真删,数它们答不出这个 client 曾经做过什么,见 [02 §3.11](./02-public-api.md)。**审计链因此不会被删除动作打断**:能删的行,本来就不在任何一条历史里。
