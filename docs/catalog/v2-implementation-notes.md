...
95. **`REQUIRE_DB_TESTS` was a knob that lied, in 75 files** (2026-08-28). The flag exists so that a run which was supposed to have a database fails instead of skipping, and `dbtest.DSN()` implements it — but 75 of the 76 files reading `TEST_DATABASE_DSN` read `os.Getenv` directly and never reached it. Measured on the W2 branch: `REQUIRE_DB_TESTS=1` with no DSN printed `ok api/internal/platform/apiv2/handler 0.305s` for a wave of gate, fence, ETag and limiter tests that ran nothing. The two heaviest suites in this review, `catalog/service` and `catalog/handler`, were both in the set, so every "I ran it with the gate on" in this review rested on the DSN happening to be set. Worse than skipping: **30 of those files fell back to a hard-coded `host=localhost … dbname=kun_catalog_test` DSN**, and all six such databases exist on the development box — so the flag did not merely fail to fire, it silently pointed the suite at a shared database another track may be mid-run against, which is the exact failure `dbtest`'s package comment forbids. All 75 now route through `dbtest`, the fallback literals are deleted, and the post-DSN refusals (unreachable server, failed migration) go through the new `dbtest.SkipMainf`/`Skipf`, which fail under the flag rather than exiting 0. Both directions are checked: nine packages across all four call shapes FAIL with the flag set and no DSN, and `go test ./...` with no DSN and no flags stays green, which is the `unit` job's contract.
...
98. **`/v2/news` ignores `lane=`, `source=`, `published_after=` and `published_before=` — found, not fixed** (2026-08-28). `Catalog.ListNews` passes a hard-coded empty `newssvc.FeedFilter{}`, while the service's `FeedFilter` supports `Sources`, `Lanes`, `PublishedAfter` and `PublishedBefore`. The forum sets all four: `newsclient.FeedQuery.values()` sends `lane`, `source`, `published_after` and `published_before`, and `internal/news/handler/news_handler.go` plus the month/archive services populate them from user input. Because none of the four is a **declared** parameter on `/v2/news`, `SkipValidateParams` drops them without a word and the caller receives an unfiltered feed. This is left for a decision rather than fixed here: closing it means adding four query parameters to a public operation, which is a contract addition and not a test wave's call. Until then a downstream filtering the news feed must filter it client-side.

99. **Two more findings recorded rather than fixed** (2026-08-28). **`/v2/me/proposals` and `/v2/moderation/proposals` parse with `collect.ClaimSpec`**, so their `fields=` vocabulary is the claim record's — `fields=state` is accepted and `fields=patch` is a 400 `UNKNOWN_FIELD` on a face whose items are proposals. It is also where their `NoBatch` refusal comes from, so changing the spec changes two behaviours at once; the batch guard above names the current source explicitly so the coupling is visible. **`internal/platform/trust/**` is not gofmt-clean on `main`** — nine files, all pre-existing (verified against `HEAD`), all unrelated alignment. No gate runs gofmt, so this is invisible to CI; it is left alone rather than swept, because a whole-tree `gofmt -w` inside a test wave is exactly the unreviewable diff this file exists to prevent.
...
checks rather than about the surface. Two API defects fell out of building the
checks, both on `/v2/news` and both the same shape — a collection parameter
declared, parsed and then ignored (deviations 90 and 91).

**Spec is 2.2.0.** No operation, parameter, schema or response moved: the only
difference from 2.1.0 is `info.version`. `oasdiff` reports "no breaking changes"
and the other eight specs report "no changes detected".
